...
export function ProductCard({ product }: { product: ProductListItem }) {
  ...
  const installment = Number(product.price ?? 0) / 6

  return (
    <Link
      href={`/produto/${product.id}`}
      className="group flex h-full flex-col overflow-hidden rounded-[1.25rem] bg-white p-2.5 shadow-[0_10px_30px_-24px_rgba(15,23,42,0.45)] ring-1 ring-slate-200/80 transition-transform duration-200 hover:-translate-y-0.5 sm:rounded-[1.75rem] sm:p-3"
    >
      <div className="relative overflow-hidden rounded-[1rem] bg-[#f4f5f7] sm:rounded-[1.25rem]">
        ...
        <button
          type="button"
          className="absolute right-2.5 top-2.5 inline-flex h-7 w-7 items-center justify-center rounded-full bg-white/95 text-slate-600 shadow-sm transition-colors hover:text-slate-900 sm:right-3 sm:top-3 sm:h-8 sm:w-8"
        >
      ...
      <div className="flex flex-1 flex-col gap-1.5 px-0.5 pb-0.5 pt-2.5 sm:gap-2 sm:px-1 sm:pb-1 sm:pt-3">
        ...
        <div className="mt-auto space-y-0.5 sm:space-y-1">
          ...
          <p className="text-[12px] text-slate-600 sm:text-sm">6x de {formatMoney(installment)}</p>
        </div>
      </div>
    </Link>
  )
